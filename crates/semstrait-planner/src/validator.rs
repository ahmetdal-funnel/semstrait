//! ConstraintValidator — pre-resolution validity gate (step 0).
//!
...
/// Map `Aggregation` to the constraint-compatible name.
/// Matches the YAML constraint format (SUM, AVG, COUNT, COUNT_DISTINCT, MIN, MAX).
fn agg_constraint_name(agg: Aggregation) -> &'static str {
    match agg {
        Aggregation::Sum => "SUM",
        Aggregation::Avg => "AVG",
...
impl ConstraintValidator {
            ...
            if let Some(measure) = iface.measures.get(measure_name) {
                if let Some(ref constraints) = measure.constraints {
                    ...
                    Self::check_aggregation_constraints(
                        measure_name,
                        constraints,
                        Some(agg_constraint_name(measure.agg)),
                    )?;
                }
            }
            // Then check metrics.
    ...
    ///
    /// **Deprecated**: Legacy helper retained for tests. Production code reads `measure.agg` directly.
    #[cfg(test)]
...
mod tests {
    ...
    fn test_one_of_satisfied() {
        ...
        let result = ConstraintValidator::check(&request, &manifest);
    ...
    fn test_extract_agg_function() {
        assert_eq!(ConstraintValidator::extract_agg_function("SUM(amount)"), Some("SUM"));
        assert_eq!(ConstraintValidator::extract_agg_function("COUNT_DISTINCT(id)"), Some("COUNT_DISTINCT"));
        assert_eq!(ConstraintValidator::extract_agg_function("AVG(price)"), Some("AVG"));
        assert_eq!(ConstraintValidator::extract_agg_function("plain_column"), None);
        assert_eq!(ConstraintValidator::extract_agg_function(""), None);
    }
