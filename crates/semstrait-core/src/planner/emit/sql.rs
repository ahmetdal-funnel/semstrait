...
use crate::planner::ir::expr::{Expr, Column, Literal, AggregateExpr};
...
fn emit_scan(scan: &Scan, indent: usize) -> Result<String, EmitError> {
    ...
    let cols = if scan.columns.is_empty() {
        ...
        scan.columns.iter().map(|c| quote_ident(c)).collect::<Vec<_>>().join(", ")
    };

    let alias = scan.alias.as_deref().unwrap_or(&scan.table);
    if alias != scan.table {
        Ok(format!("{p}SELECT {cols}\n{p}FROM {} AS {}",
            quote_ident(&scan.table), quote_ident(alias)))
    } else {
        Ok(format!("{p}SELECT {cols}\n{p}FROM {}",
...
fn emit_join(join: &Join, indent: usize) -> Result<String, EmitError> {
    ...
    let on_clause: Vec<String> = join.left_keys.iter().zip(join.right_keys.iter())
        .map(|(lk, rk)| format!("{} = {}", emit_column(lk), emit_column(rk)))
        .collect();
...
fn emit_column(col: &Column) -> String {
    if col.table.is_empty() {
        ...
        format!("{}.{}", quote_ident(&col.table), quote_ident(&col.name))
    }
}

/// Quote an SQL identifier using ANSI double-quote style.
...
mod tests {
    use super::*;
    use crate::planner::ir::expr::BinaryOperator;
    use crate::planner::ir::plan_node::{ProjectExpr, SortKey};
