...
#[derive(Debug)]
#[allow(dead_code)] // Variants used in tests and sql_emitter
pub enum PlanNode {
    /// Scan a table
    Scan(Scan),
    /// Join two relations
...
pub struct Join {
    ...
    /// Left key columns (one per join column pair)
    pub left_keys: Vec<Column>,
    /// Right key columns (one per join column pair)
    pub right_keys: Vec<Column>,
}
...
/// Sort direction
#[derive(Debug, Clone, Copy, PartialEq)]
#[allow(dead_code)] // Used in sql_emitter tests
    ...
    fn fmt_indent(&self, f: &mut fmt::Formatter<'_>, indent: usize) -> fmt::Result {
        ...
        match self {
            ...
            PlanNode::Join(join) => {
                ...
                let keys: Vec<String> = join.left_keys.iter().zip(join.right_keys.iter())
                    .map(|(l, r)| format!("{} = {}", l.qualified_name(), r.qualified_name()))
                    .collect();
                writeln!(f, "{}{}Join: {}",
                    pad, jt, keys.join(" AND "))?;
