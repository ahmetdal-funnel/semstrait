...
pub(crate) fn emit(
    ...
    let total = model.entities.len();
    tracing::info!("semstrait emit: compiling {} entities", total);
    let mut entities = IndexMap::with_capacity(total);
    for dk in model.entities.values() {
...
fn extract_identifiers_from_expr(expr: &str) -> Vec<String> {
    static SQL_KEYWORDS: std::sync::OnceLock<HashSet<&'static str>> = std::sync::OnceLock::new();
    let sql_keywords = SQL_KEYWORDS.get_or_init(|| {
        [
