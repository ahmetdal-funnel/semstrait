...
use super::expr::{build_filter_expr, convert_measure_expr, convert_metric_expr};
use super::util::{
    build_attribute_expr, build_column, build_sort_keys, collect_required_columns,
    extract_physical_dims, needs_join_for_dimension, prefixed_subquery_col, semantic_path_as_key,
};
use crate::plan::{
    Aggregate, AggregateExpr, Column, CrossJoin, Expr, Filter, Join, JoinType, Literal, PlanNode,
    Project, ProjectExpr, Scan, Sort,
};
use crate::resolver::{ResolvedDimension, ResolvedQuery};
...
pub fn plan_query(resolved: &ResolvedQuery<'_>) -> Result<PlanNode, PlanError> {
    ...
    if !resolved.filters.is_empty() {
        let filter_exprs: Vec<Expr> = resolved
            .filters
            .iter()
    ...
    let group_by: Vec<Column> = resolved
        .row_attributes
        .iter()
    ...
    let aggregates: Vec<AggregateExpr> = resolved
        .measures
        .iter()
...
pub fn build_grain_set_branch_for_dataset(
    ...
    let measure_names: Vec<&str> = measure_aliases.iter().map(|(_, m)| m.as_str()).collect();

    if let Some(table) = target_dataset {
        build_single_table_aggregate(
            model,
            grain_set,
            table,
            &physical_dims,
...
pub fn build_single_table_aggregate(
    ...
    let group_by: Vec<Column> = physical_dims
        .iter()
    ...
    let aggregates: Vec<AggregateExpr> = measure_aliases
        .iter()
    ...
    let aggregated_aliases: HashSet<String> =
    ...
    for (idx, (dim_name, attr_name)) in physical_dims.iter().enumerate() {
        ...
        let output_name = if let Some(prefix) = output_prefix {
            prefixed_subquery_col(prefix, &semantic_path_as_key(&semantic_name))
        } else {
            semantic_name
        };
    ...
    for (alias, measure_name) in measure_aliases {
        let output_name = if let Some(prefix) = output_prefix {
            prefixed_subquery_col(prefix, alias)
        } else {
            alias.clone()
        };
        if aggregated_aliases.contains(alias.as_str()) {
            projections.push(ProjectExpr {
                expr: Expr::Column(Column::unqualified(alias)),
                alias: output_name,
            });
        } else {
            let data_type = grain_set
                .get_measure(measure_name)
                .map(|m| m.data_type().to_string())
...
fn build_multi_table_aggregate(
    ...
    if sub_queries.len() == 1 {
        ...
        for (dim_name, attr_name) in physical_dims {
            let semantic_name = format!("{}.{}", dim_name, attr_name);
            let prefixed = prefixed_subquery_col(&prefix, &semantic_path_as_key(&semantic_name));
            projections.push(ProjectExpr {
        ...
        for (alias, measure_name) in measure_aliases {
            if assigned_measures.contains(measure_name.as_str()) {
                let prefixed = prefixed_subquery_col(&prefix, alias);
                projections.push(ProjectExpr {
                    expr: Expr::Column(Column::unqualified(&prefixed)),
                    alias: alias.clone(),
                });
    ...
    for (right_plan, right_alias) in sub_queries {
        if let Some((dim_name, attr_name)) = physical_dims.first() {
            let semantic_name = format!("{}.{}", dim_name, attr_name);
            let key_base = semantic_path_as_key(&semantic_name);
            let left_col_name = prefixed_subquery_col(&left_alias, &key_base);
            let right_col_name = prefixed_subquery_col(&right_alias, &key_base);
    ...
    for (dim_name, attr_name) in physical_dims {
        ...
        let key_base = semantic_path_as_key(&semantic_name);
        let coalesce_args: Vec<Expr> = table_indices
            .iter()
            .map(|idx| {
                let col_name = prefixed_subquery_col(&format!("t{}", idx), &key_base);
    ...
    for (alias, measure_name) in measure_aliases {
        let table_idx = table_measures
            .iter()
            .find(|(_, measures)| measures.iter().any(|(_, m)| m == measure_name))
            .map(|(idx, _)| *idx);
        if let Some(idx) = table_idx {
            let col_name = prefixed_subquery_col(&format!("t{}", idx), alias);
            projections.push(ProjectExpr {
