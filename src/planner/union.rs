...
use super::util::{
    get_virtual_attribute_value, get_virtual_attribute_value_with_dataset,
    needs_join_for_dimension, ParsedDimensionAttr,
};
use crate::plan::{
    Aggregate, AggregateExpr, Column, Expr, Join, JoinType, Literal,
    LiteralValue as PlanLiteralValue, PlanNode, Project, ProjectExpr, Scan, Union, VirtualTable,
};
use crate::query::QueryRequest;
use crate::resolver::{collect_required_measure_names, resolve_query};
use crate::selector::{
    select_datasets_for_join, select_partial_for_grain_set, PartialSelection, SelectedDataset,
};
...
pub fn plan_conformed_query(
    ...
    let physical_dims: Vec<String> = dimension_attrs
        .iter()
        .filter(|d| {
    ...
    let virtual_dims: Vec<String> = dimension_attrs
        .iter()
        .filter(|d| {
            let parts: Vec<&str> = d.split('.').collect();
            if parts.len() != 2 {
    ...
    let is_feasible_table = |grain_set: &GrainSet, table: &Dataset| -> bool {
        ...
        for measure_name in &required_measures {
            if grain_set.get_measure(measure_name).is_none() {
                return false;
            }
    ...
    for grain_set in &grain_sets {
        ...
        if let Some(table) = feasible_table {
            ...
            let resolved = resolve_query(schema, request, &selected, &grain_sets).map_err(|e| {
        ...
        if let Ok(multi) =
            select_datasets_for_join(schema, model, gs_slice, dimension_attrs, &required_measures)
        {
            if multi.datasets.len() == 1 {
                ...
                let branch =
                    plan_same_grain_set_join(model, &multi, dimension_attrs, &metric_names)?;
                branches.push(branch);
            }
            continue;
        }
        ...
        if let Some(partial) =
            select_partial_for_grain_set(model, grain_set, dimension_attrs, &required_measures)
        {
            let branch =
                build_partial_union_branch(model, &partial, dimension_attrs, &metric_names)?;
            branches.push(branch);
        }
    }
...
pub fn plan_multi_grain_set_query(
    ...
    for grain_set in &grain_sets {
        ...
        if let Some(table) = feasible_table {
            let branch =
                build_union_branch(model, grain_set, table, dimension_attrs, &metric_names)?;
            branches.push(branch);
            continue;
        }
...
pub fn plan_single_grain_set_query(
    ...
    let grain_sets = model.grain_sets();
    let grain_set = grain_sets
        .iter()
        .find(|tg| tg.name == target_group)
        .ok_or_else(|| {
            PlanError::InvalidQuery(format!("Grain set '{}' not found in model", target_group))
        })?;

    let metric_names: Vec<String> = request.metrics.clone().unwrap_or_default();
    let required_measures = collect_required_measure_names(model, &metric_names);
    let feasible_table =
        find_feasible_table_for_qualified(model, grain_set, dimension_attrs, &required_measures);
    ...
    let normalized_dims: Vec<String> = dimension_attrs
        .iter()
        .map(|path| {
                        ...
                        .map(|r| {
                            r.iter().any(|rd| {
                                ...
                                if parts.len() >= 3 {
                                    ...
                                    model.grain_set_under_path(path_segments, target_group)
                                        && normalized == **d
                                } else {
                                    rd == *d
                                }
    ...
    let resolved =
        resolve_query(schema, &normalized_request, &selected, &grain_sets).map_err(|e| {
            PlanError::InvalidQuery(format!(
                "Query resolution error for tableGroup '{}': {:?}",
                target_group, e
            ))
...
fn build_union_branch(
    ...
    let measures_to_aggregate: Vec<(&str, &crate::semantic_model::Measure)> = metric_names
        .iter()
        .filter_map(|metric_name| {
            model.get_metric(metric_name).and_then(|m| match &m.expr {
                MetricExpr::MeasureRef(measure_name) => grain_set
    ...
    let aggregates: Vec<AggregateExpr> = measures_to_aggregate
        .iter()
        .map(|(metric_name, measure)| AggregateExpr {
...
fn build_partial_union_branch(
    ...
    let missing_dims: HashSet<&str> = partial
        .missing_dimensions
        .iter()
        .map(String::as_str)
        .collect();
    let present_dims: HashSet<&str> = partial
        .present_dimensions
        .iter()
        .map(String::as_str)
        .collect();
    let present_measures_set: HashSet<&str> = partial
        .present_measures
        .iter()
        .map(String::as_str)
        .collect();
    ...
    let physical_attrs: Vec<&(String, ParsedDimensionAttr)> = parsed_attrs
        ...
        .filter(|(path, parsed)| {
            ...
            parsed.belongs_to_grain_set(model, &grain_set.name)
                && present_dims.contains(path.as_str())
        })
        .collect();
...
pub fn plan_virtual_only_query(
    ...
    let attrs: Vec<(&str, &str)> = virtual_dims
        .iter()
        .filter_map(|d| {
            let parts: Vec<&str> = d.split('.').collect();
            if parts.len() == 2 {
    ...
    for grain_set in model.grain_sets() {
        ...
        let row: Vec<PlanLiteralValue> = attrs
            .iter()
            .map(|(dim_name, attr_name)| match first_dataset {
                Some(ds) => get_virtual_attribute_value_with_dataset(
                    model,
                    &grain_set,
                    Some(ds),
                    dim_name,
