...
| `COMP_E_2102` | `SimpleSourceGlobEmpty { data_kind, pattern }` | The Simple's Binding's source glob expanded to zero sources. Semantically equivalent to `COMP_E_0301 NoSourcesMatched` per `15 §3.5.2`; `21` re-surfaces under the Simple's diagnostic context for the common "the author meant this Simple's glob, show the author which Simple" pattern. (Whether `21` re-surfaces or passes through verbatim is the Q-DS-002 open item.) |
