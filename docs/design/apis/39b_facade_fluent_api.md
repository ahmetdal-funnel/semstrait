...
| `ExprSource` / `ExprBlock` (YAML authoring surface) | enum / enum | `semstrait-model` | `[14 §6](../foundations/14_expressions.md)` (contract) + `[32](32_semstrait_model.md)` (impl) |
...
|---|---|---|
| `semstrait-core` | Primitives (`DataType`, `Grain`, `Schema`); diagnostic primitives (`Diagnostic<K>`, `Severity`, `Location`, `Span`, `SourceId`); trait scaffolding (`Tree`, `ExprLeaf`, `Visitor`, `Rewriter`); support enums (`BinaryOpKind`, `UnaryOpKind`, `AggregationOp`, `LikeKind`, `CastFailure`) | Any canonical-IR type (no `Expr<L>`, no `PhysicalExpr`, no `CanonicalFn`); any model / manifest / plan type |
| `semstrait-ir` | The canonical IR layer: `Expr<L>` + leaf sets + type aliases (`PhysicalExpr`, `SemanticExpr`); `Accessor` family + `Parameter`; `CanonicalFn` + `FunctionRegistry`; `PlanNode` + `SemanticPlan` + `EngineArtifact`; expression DSL (free constructors in `expr_fn`; `std::ops` impls; aggregate/window builder traits) | Parsing (no YAML, no inline DSL parser); compile resolution; planning algorithms; engine emission |
| `semstrait-model` | **The YAML surface** (`ExprSource`, `ExprBlock`, parse-site dispatch); **parse-time structural rules** (per-site shape gates, identifier syntax, reserved-tag catalog); **entity types** (`SemanticModel`, `Dataset`, `Grainset`, `Unionset`, `Joinset`, `Dimension`, `Measure`, `Metric`, `Relationship`, `Filter`, `Keys`, `SemanticInterface`, `SemanticMapping`); validate-stage checks (cross-entity SR-* rules) | The canonical expression *semantics* (consumes `SemanticExpr` / `PhysicalExpr` from `semstrait-ir` as value types — does not own their shape evolution); compile-time resolution (no name lookup, no cross-DataKind path BFS); function-registry evolution |
| `semstrait-manifest` | Compile-time resolution (`SemanticExpr` → `PhysicalExpr`); `ResolvedExprTable`; `SemanticManifest` (sealed artifact); `Repository` trait + bundled impls; the `Compile` extension trait (build → compile transition) | Authoring (no YAML parsing); planning (no `Request` handling) |
