...
> **Status:** ratified. `31` carries the **non-expression shared vocabulary** of the workspace: the logical-type system (`DataType`, `Grain`, `TypeClass`, `Schema`, `SchemaColumn`), the constraint DSL, the cross-cutting diagnostic primitives, and the `io` transport per `31b`. Every expression-tied type — traits, support enums, literal carriers, identifier carriers, narrow ir-emitted error kinds — lives in `semstrait-ir` per `[35](35_semstrait_ir.md)`.
...
- **Every expression-tied type.** Traversal traits (`Tree`, `Visitor<N>`, `Rewriter<N>`, `ExprLeaf`), `Expr<L>` and its leaf sets, accessor enums, `Parameter`, support enums (`BinaryOpKind`, `UnaryOpKind`, `AggregationOp`, `LikeKind`, `CastFailure`, `WindowFn`, `WindowFrame`, `WindowFrameKind`, `WindowBound`), `Literal`, identifier carriers (`ColumnRef`, `SemanticsName`), the `expr_fn` DSL, `CanonicalFn` / `FunctionRegistry`, and the narrow ir-emitted error kinds (`ValidateError`, `CompileError`). All in `semstrait-ir` per `[35](35_semstrait_ir.md)`. Downstream stages embed the narrow errors via D.ii kind-nesting (`semstrait-model::ValidateError` carries `Ir(ir::ValidateError)`; `semstrait-manifest::CompileError` carries `Ir(ir::CompileError)`).
- **The YAML authoring surface.** `ExprSource` and the reserved-tag dispatch live in `semstrait-model` per `[14 §9.3](../foundations/14_expressions.md)` and `[32](32_semstrait_model.md)`. `ExprSource::Block(...)` carries `Expr<L>` directly via serde derives — no separate `ExprBlock` type.
