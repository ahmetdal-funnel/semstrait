...
- **§2.1** — MINOR is strictly additive (new `#[non_exhaustive]` variant, new `#[non_exhaustive]` field, new public symbol, `#[deprecated]` attribute, additional reserved canonical `tracing` field name). MAJOR is anything non-additive. PATCH is behavior-preserving.
- **§2.2** — Renaming or removing a per-stage kind variant is MAJOR; adding `#[deprecated]` to a variant is MINOR. Variants stay callable / matchable for at least one MINOR cycle after deprecation.
...
- **§4.1** — The public sum-type roster is `#[non_exhaustive]`: canonical domain enums (`DataType`, `DataKind`, `Additivity`, `Cardinality`, `JoinType`, `DialectId`, `EngineArtifact`, `EnginePlan`, `ExprSource` variants, `TemporalShape` + SCD subtype, composition-kind tag, `DimensionType`, `Grain`, `LiteralValue`, `BinaryOpKind`, `Aggregation`, `FunctionCategory`, `ParamType`, `ReturnTypeRule`, `Portability`), `Severity`, every per-stage kind enum (`ParseErrorKind`, `ValidateErrorKind`, `CompileErrorKind`, `PlanErrorKind`, `OptimizeErrorKind`, `AdaptErrorKind`, `CatalogProviderErrorKind`, `FileSystemErrorKind`, `RepositoryErrorKind`, `SemStraitErrorKind`).
- **§4.1** — `CanonicalFn` is a newtype; inherently extensible; no `#[non_exhaustive]` annotation needed. `SourceId` is opaque (no `#[non_exhaustive]` annotation needed; its variant set is private to the producing crate).
- **§4.2** — `Diagnostic<K>`, `Location`, `FunctionSpec`, `SemanticManifest` + `Resolved*` family, `SemanticPlan`, `PlanNode` sub-structs, `SemanticInterface`, `ComposedSemanticInterface`, `Request`, `SessionContext`, `SqlArtifact` are `#[non_exhaustive]` public structs.
- **§4.3** — Internal-only (`pub(crate)` or narrower) enums MAY be exhaustive.
- **§4.4** — Library code never panics on an unknown non-exhaustive variant; wildcard arms return `Diagnostic<K>`s carrying enough context to debug.
- **§5.1** — `Diagnostic<K> { kind, severity, location, notes }` is the canonical envelope; generic over the per-stage kind type. `Diagnostics<K> = Vec<Diagnostic<K>>`. `#[non_exhaustive]`.
- **§5.1** — No central error-code allocation table. Stable identification is per-stage kind variant identity. The earlier `{SUBSYSTEM}_{SEVERITY}_{NUMBER}` framework is **retired**.
- **§5.2** — `Severity ∈ {Error, Warning}`; `#[non_exhaustive]`. The earlier `Info` variant retired (informational signals belong on the `tracing` channel of §6).
- **§5.3** — `Location { source: SourceId, span: Span }` lives in `semstrait-core`. `SourceId` is opaque with private constructors. `ContextLine` is **retired** — short remarks go in `Diagnostic.notes`; structured cross-references go in the kind variant.
- **§5.4** — `Diagnose` is the trait every per-stage kind implements. Provides `message()`, `severity_default()`, optional `cause()`. `Diagnostic<K>` has blanket `Display` and `std::error::Error` impls via `Diagnose`.
- **§5.4** — Foreign-error wrapping is variant-side: typed variants on per-stage kinds (e.g. `ParseErrorKind::Yaml(serde_yaml::Error)`); chains via `Diagnose::cause()`.
- **§5.5** — Raw-string errors, `anyhow::Error`, `Box<dyn Error>`, untyped `Result<T, Diagnostic>` (without `K`) are banned on public APIs. Panics on caller-reachable input are banned. Direct stdout/stderr writes are banned.
- **§5.6** — Cross-stage aggregation uses a sum-typed kind (`SemStraitErrorKind`) at fused helpers (`38`); per-stage results lift via `From` impls.
- **§6** — Workspace observability is via `tracing` (no `log`, no custom event channel). No `semstrait-*` library crate writes to stdout / stderr.
- **§6.2** — Every `pub` stage entry-point carries `#[tracing::instrument]`; spans propagate across `.await` automatically.
- **§6.3** — Level discipline: `error` reserved for internal bugs; `warn` for library-internal advisories not surfaced as Diagnostics; `info` for what-is-happening signals; `debug` / `trace` for developer detail.
- **§6.4** — Diagnostic ↔ tracing orthogonality: the library never double-emits. Diagnostics flow through `Result`; tracing events flow side-channel.
- **§6.5** — Canonical reserved `tracing` field-name vocabulary: `stage`, `entities`, `sources`, `duration_ms`, `engine`, `adapter`, `kind`. Authors may add others; canonical names are reserved.
- **§6.6** — Subscriber configuration is the embedder's responsibility; library crates do not depend on a subscriber crate.
- **§7.1** — Per-stage classification: `parse` / `validate` accumulating; `compile` / `plan` / `optimize` / `adapt` / `CatalogProvider::resolve` / `FileSystem::read` / `Repository::{load,save}` / fused helpers fail-fast.
- **§7.2** — Stage return shapes:
  - Accumulating: `Result<(Output, Diagnostics<K>), Diagnostics<K>>`.
  - Fail-fast: `Result<(Output, Diagnostics<K>), (Diagnostic<K>, Diagnostics<K>)>`.
  - Special case: `validate` returns `Result<Diagnostics<K>, Diagnostics<K>>` (no Ok-tuple wrapper, since output is `()`).
- **§7.3** — Warnings are never silently dropped; both arms of every stage signature carry them.
- **§7.4** — Stage of origin is the kind type, not a substring of a code. Cross-stage kind nesting (`CompileErrorKind::Parse(ParseErrorKind)`) is permitted but not mandatory.
- **§7.5** — `parse` / `validate` accumulate without an intrinsic limit at the `30` level; per-crate docs MAY introduce a soft cap with a sentinel kind variant.
- **§7.6** — Public entry points never panic on caller-reachable input.
- **§8.1** — Sealed public traits use the private-super-trait pattern.
- **§8.2** — Trait roster: `CatalogProvider`, `FileSystem`, `Repository`, `EngineAdapter`, `RegistryExtension`, `Diagnose` — all open in v1.
- **§8.3** — Public trait methods return per-stage typed shapes from §7.2 (`Result<(T, Diagnostics<K>), …>` with `K` being the trait-owning crate's kind enum).
