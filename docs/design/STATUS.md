...
- Id-first identity rework (items U / U.2) — **follow-ups landed (2026-05-29).** Single `EntityId` lane across model, manifest, and runtime: `RelationshipId` and `BindingId` newtypes eliminated tree-wide (`15`/`16`/`18`/`19`/`24`/`33`/`34`/`35`); first-match-wins + cross-kind BFS + Grainset routing tie-breaks reconciled to **name order** (`16`/`19`/`22`/`23`/`25`); UUID-format ratified (authored UUIDv7, compile-generated UUIDv5 with per-entity namespaces — `closed/33` Q-MAN-D12). Composition `ImplicitId` is now stable across recompiles when relationship ids are stable (`16 §5.7`; authored/persisted ids — Convenience-profile caveat noted). `Q-MAP-001` mooted (`closed/15`). Remaining graph-structure handles (`SemanticNodeId`/`SemanticEdgeId`, `bit_position`) and the expr-pool id (`PhysicalExprId`) are intentionally retained. **Still open (pre-existing item-T drift, NOT part of the id rework):** `15 §7`'s `ResolvedColumnMapping` / `ResolvedBinding` manifest-layer narrative + the broken `33 §5.3` cross-ref describe a manifest shape superseded by `33 §7`'s `SemanticBinding` + `ManifestExpressions`; reconcile in a dedicated `15`↔`33` pass. **Deferred enhancements (from review):** normative UUIDv5 seed byte-encodings (binding + implicit-Unionset) in `33 §9.1` for a tight I4 proof; CX1 derived-view consistency checks (canonical `bit_position` contiguity; bitmask ↔ member-set equality).
...
| U | Id-first named-entity identity rework (`32` / `18` + cascades) — supersedes the `SemanticModelIdentityIndex` sidecar + `EntityHandle`/`SemanticModelStorage` builder machinery introduced in commit `0b519c3`. Identity is now an `id: EntityId` field carried directly on every named entity (`DataKindBase`; `Dimension`/`Measure`/`Metric`/`Relationship`/`Key`/`Filter`; `*Ref` ref-sites). Every model collection — top-level data kinds, shared pools, nested children, relationships, and `SemanticInterface`/`Keys`/aggregation-filter member collections — is a `BTreeMap<EntityId, _>`; the `id` is the storage key. Iteration/serialization is name-ordered (`32 §7`) so I4 stays byte-stable. Duplicate-by-name is an explicit per-layer scan at `.build()` (`32 §9.0`). **Consequences (resolved 2026-05-29):** (1) relationship first-match-wins and BFS neighbor iteration now resolve in name order (reconciled across `16`/`18`/`19`); (2) Unionset/Grainset intra-variant child order for `PlanNode::Union` inputs now name-ordered (`22 §3.1` / `23 §3.1` / `33 §6.5`). `RelationshipId` eliminated entirely (see item U.2). | Complete | `apis/32_semstrait_model.md`, `foundations/18_entities.md`, `apis/33_semstrait_manifest.md`, `data-kinds/{22,23,24,26}_*.md`, `INDEX.md` |
| V | Layered expressions in manifest + semantic graph (`14`/`14a`/`19`/`33`/`34`/`35` + `22`/`23`) — 5 committed iterations (2026-05-29). **(1)** `ExprLayer { Scalar, Aggregate, PostAggregate }` (concept `14 §3.8`, type `35 §5.5`) — the applicability layer of a lowered `PhysicalExpr`'s result; tree-derived, decoupled from `SemanticRole`; assigned at compile (`19 §3.2.6`). **(2)** Manifest is **physical-only**: `ManifestExpressions { physical: BTreeMap<PhysicalExprId, ManifestExpression{ expr, layer }> }` — semantic pool + `SemanticExprId` dropped (supersedes C11 "both pools" / C12.3 split / C12.5; C16 reverse-index now physical). **(3)** Additivity is **function-derived in v1** behind `AdditivitySource` (`14a §3.6.2`) with branching once on `Additivity` (`§3.6.1`); model-level deferred (reserved). **(4)** Phase B placement keyed on `ExprLayer` (`19 §6.0`); `34`/`22`/`23` consume it. **(5)** Graph reserves `Expression`-node/`DependsOn` (`35 §2A.1`) for post-v1 (Gap C: tree + layer + additivity suffice; Metric→Measure decomposition recovered by aggregate-lift, no parallel structure). CX1 validates `layer == classify(expr)`. **Open:** ratify the C11/C12 supersession + the `19 §6.5` model-level additivity extension when logical/semantic additivity lands. | Complete (spec) | `foundations/{14,14a,19}_*.md`, `apis/{33,34,35}_*.md`, `data-kinds/{22,23}_*.md`, `INDEX.md` |
| U.2 | Manifest single-id-lane unification (`33` + cascades) — extends item U into the manifest. Deletes the `ManifestStableIds` side-map and removes the per-kind compile-id newtypes (`DataKindId`/`SemanticsId`/`SemanticInterfaceId`/`BindingId`/`SourceId`/`RelationshipId`) from the manifest. Every manifest collection is `BTreeMap<EntityId, _>`, every cross-reference is an `EntityId`, and each entity carries `id: EntityId` (its key); top-level `model_id` added. The lone surviving integer is the bitmap `bit_position` ordinal (`SemanticBitmap` keyed by `EntityId`, reverse `bit_position→EntityId` derived at load; word-math unchanged). Compile-synthesised entities (sources, bindings, interfaces, implicit Unionsets, synthesised fields, `model_id`) get **deterministic content-derived** ids to preserve I4 — requiring a UUID-format relaxation (authored = UUIDv7, generated = deterministic variant). Expression-pool ids (`PhysicalExprId`; the `SemanticExprId` was later dropped by item V) are the one deliberate exception (content-dedup handles). Cascades: `35 §2A` graph payload refs + `§15.4` wire line → `EntityId`; `18 §2.1` `RelationshipId` **eliminated**; `15 §2.2` binding identity = `EntityId` (`BindingId` eliminated). **Follow-ups landed (2026-05-29):** UUID-format ratified (UUIDv5 + namespaces, `closed/33` Q-MAN-D12); `RelationshipId`/`BindingId` fully eliminated across `15`/`16`/`19`/`24`/`34`/`35`; name-order reconciliation complete; composition `ImplicitId` now stable across recompiles. | Complete | `apis/33_semstrait_manifest.md`, `apis/35_semstrait_ir.md`, `foundations/{15,16,18,19}_*.md`, `data-kinds/{22,23,24,25}_*.md`, `apis/{32,34}_*.md`, `INDEX.md` |
...
| W | Manifest relationship + temporal reconciliation (item‑T drift fix) — `33 §8A` defines compile-resolved `ResolvedRelationship` / `ResolvedJoinKey` (replacing the bare spec‑18 re-export: `EntityId` endpoints, materialized `optional`/`cross_filter`, derived `join_type`, semantic-id join keys, pooled `filter`); `33 §8B` propagates `TemporalShape`/`TemporalType` onto `DataKind`/`NestedDataKind.temporal` (dim refs → `EntityId`, generic `grain`, leaf-only). Relationship key/filter lower via Phase A + intern in `ManifestExpressions` (`19 §3.12`; computed keys → synthesised `Field`). Dangling `ResolvedRelationship (33 §8.1)` / `ResolvedTemporalShape` refs repointed across `15`/`16`/`17`/`18`/`24`/`34`; `16 §4` self-contradiction fixed. **Out of scope:** `15 §7`↔`33 §7` (`ResolvedColumnMapping`/`ResolvedBinding`) pass; `16 §10`/`§11` composition-persistence prose (Phase 3.5b); code convergence (`CompiledManifest`/`CompiledRelationship` still pre-spec). | Complete (spec) | `33`, `19`, `15`, `16`, `17`, `18`, `24`, `34`, `INDEX.md` |
...
3. **Phase 2 research — Target C (Iceberg / Delta-Lake schema-evolution model).** Highest-leverage queued target; informs C3 (PhysicalSource fields), C14 (catalog fetch), C15 (schema fingerprint). Then Targets A (small-cardinality bitmap registries; informs C4–C8), B (dual expression-form persistence; informs C11/C12 — **superseded for v1 by item V's physical-only pool; reframe as the post-v1 semantic-form/provenance question**), D (hop-depth caps; informs C10), E (composition-graph cycle detection; informs C13 G1).
