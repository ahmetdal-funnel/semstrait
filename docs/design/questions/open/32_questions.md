...
**Next step.** `[TD-MODEL-DIR-LOADER]` remains open and is carried over to `31b_io_questions.md` (`Q-IO-003`), since the multi-file loader design is a CLI / LSP concern that depends on `core::io`'s back-end roster. No action in `32` until a concrete CLI spec surfaces.
