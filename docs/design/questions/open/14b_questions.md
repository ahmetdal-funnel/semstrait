...
status: Living
purpose: Round-1 open / proposed defaults raised against `foundations/14b_expression_resolution.md`
---

# 14b — Open Questions (Round 1)

This companion file tracks decisions from `docs/design/foundations/14b_expression_resolution.md` that were **deferred** or **proposed as Round 1 defaults with a review owed later**. Closed items moved to [`../closed/14b_questions.md`](../closed/14b_questions.md). Each entry has:
...
## OQ-1. SemanticManifest-level `PhysicalExpr` interning
...
- SemanticManifests are produced once and consumed many times; decode-time simplicity beats encode-time size reduction.
- A first-cut implementation is simpler to audit for correctness.

**Review trigger.** If SemanticManifest sizes pass a comfort threshold (proposed: >10 MB compressed for typical Models) or planner decode is measurably dominated by expression-tree allocation, revisit.

**Tech-debt tag.** `[TD-14B-EXPR-INTERN]`

**Linked docs.** `33` (SemanticManifest), §2.4.
...
- No downstream component caches IDs across compile runs; they are internal to a single SemanticManifest.
- Content-hash IDs add a build-time cost (hashing every Binding / Relationship) and a lookup cost at plan time (hash vs. `u32`).
- SemanticManifest diffability is a separate concern that can be addressed at the serialization layer (`33`) without changing the in-memory ID scheme.

**Review trigger.** If diff-based SemanticManifest caching becomes a feature (incremental re-compile, CI-level SemanticManifest diff validation), revisit. A stable-ID scheme would be a prerequisite for content-addressable SemanticManifest storage.
