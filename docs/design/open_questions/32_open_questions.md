...
The late-session Q&A sweep ratified 18 model-level decisions, consolidated into the new [`apis/32c_entities.md`](../apis/32c_entities.md) and cascaded into `32`, `32b`, `26`. Stale items retired in that pass (kept below only as historical markers per the docs-retention rule):
...
- `Q-MODEL-005` — parse-site audit table must now include the new `expr:` fields ratified in `32c §2` (`Relationship.keys[].from` / `.to`, `Relationship.filter`) and every `ref`-site override per `32c §1.3`. Still open pending the concrete audit test.
...
New questions generated by the 32c ratifications that warrant open-question entries: **zero**. The 18 decisions closed every design-level ambiguity the sweep surfaced; remaining per-variant YAML cascade work (`21`–`24` rewrites) is mechanical Phase-3 work, not new Q&A.
...
- `32c §11` — 12 new `SR-E-*` codes, all kebab-case, zero numeric counterparts.
...
- Every SR-E-* code ratified in `32c` is kebab-case-native; retrofit to numeric would require new allocations in `30 §6` for rules that never needed them.
...
**Next step.** Land a `30` amendment making kebab-case the primary across all subsystems (matching `10 §5.1` / `31 §7.4` / `32 §11` / `32c §11`). Keep `30 §6`'s numeric allocation table as documentation of the historical allocation but mark it "secondary." If the numeric form is demonstrated to be valuable for log-tooling, add `legacy_code()` accessors uniformly across subsystems; otherwise drop the comments.
...
**Expanded scope (post-2026-04-17 canonical-entity closure).** The 32c ratifications added several new parse sites that must be included in any audit table:

- **`32c §2.2` (Relationship.keys).** `JoinKeyExprPair { from: ExprSource, to: ExprSource }` — both sides are `SemanticExpr` parse sites; authored at root `relationships:` AND `JoinsetBody.relationships`.
- **`32c §2.2` (Relationship.filter).** Optional residual predicate — `SemanticExpr` parse site.
- **`32c §1.3` (reference-site `expr:` override).** Every `- ref: <name>` entry on a DataKind's `SemanticInterface` may carry a local `expr:` override (Dimension / Measure / Metric) — `SemanticExpr` parse site.
- **`32c §7.1` (DataKindFilter.expr).** `SemanticExpr` parse site — unchanged in shape from the pre-ratification Filter but now ratified canonically.
- **`32c §7.2` (AggregationFilter.expr).** `SemanticExpr` parse site — evaluated inside the aggregation wrapper at compile lowering.
- **`32c §10.1` (SemanticMapping `Expr` variant).** `PhysicalExpr` parse site — distinct from the `Column` / `Literal` variants that dispatch without parsing an expression.
...
- `32c §4` / `§5` / `§6` — Dimension / Measure / Metric `expr:` sites (all `SemanticExpr`).
- `32c §2.2` — `Relationship.keys[].from` / `.to` and `Relationship.filter` (all `SemanticExpr`).
- `32c §1.3` — `ref`-site `expr:` override (`SemanticExpr`, inherits carrier rules).
- `32c §7` — Filter taxonomy `expr:` sites (`SemanticExpr`).
- `32c §10.1` — `SemanticMappingValue::Expr(PhysicalExpr)` (`PhysicalExpr`).
...
- With `32c` adding ~six new parse-site kinds in one ratification pass, the risk of silent drift at the next extension is higher than before.
...
**Current position in `32` / `32c`.** Implicit across `32 §5.1` and every `32c §N` that carries an `expr:` field. Not tabulated explicitly in either doc.

**Next step.** Add a short appendix to a future revision of `32c` enumerating the exhaustive parse-site table (`32c` is the natural home because most parse sites are owned by `32c`). Until then, an integration test in `crates/semstrait-model/tests/` maintains the check programmatically (the test enumerates every `ExprSource` field via reflection-style match). Record as `[TD-EXPR-PARSE-SITE-AUDIT]`.
